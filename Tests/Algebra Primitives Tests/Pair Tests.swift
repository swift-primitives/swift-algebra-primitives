...
struct `Pair - Type-Specific Tests` {
    ...
    func `allFirsts when First is CaseIterable`() {
        ...
        #expect(Array(allSigns).count == 3)
        #expect(Array(allSigns).contains(.positive))
        #expect(Array(allSigns).contains(.negative))
        #expect(Array(allSigns).contains(.zero))
    }

    @Test
    func `Sign Value typealias for classifier types`() {
